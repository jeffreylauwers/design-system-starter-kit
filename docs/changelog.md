...
- **Geen lijsten en geen links meer in een description** (issue [#318](https://github.com/jeffreylauwers/design-system-starter-kit/issues/318)): de inhoud van een `aria-describedby`-koppeling wordt platgeslagen tot één tekst. Getest met VoiceOver in Safari blijkt een lijst binnen een description helemaal niet te worden voorgelezen: de items ontbreken volledig in wat de gebruiker hoort. De docs presenteerden een lijst juist als vanzelfsprekende optie, met een eigen story en voorbeelden in stories en templates. Gerelateerd succescriterium: [WCAG 1.3.1 Info en relaties](https://www.w3.org/WAI/WCAG22/quickref/#info-and-relationships).
  - Een description bevat vanaf nu alleen lopende tekst. Heb je een opsomming nodig, zet die dan als gewone `UnorderedList` boven het hele form field, buiten de `aria-describedby`-koppeling. Daar houdt de lijst zijn lijstsemantiek en wordt hij wel voorgelezen
  ...
  - De twee stories **Within Form Field: Single file** en **Within Form Field: Multiple files** van `FileInput` gebruiken nu lopende tekst in plaats van een lijst. Hetzelfde geldt voor de templates **Form step: Simple** en **Form step: All types**
  - Het template **Form step: Upload** laat de andere vorm zien: de bestandsvereisten staan als `UnorderedList` boven het form field, buiten de `aria-describedby`-koppeling, met een korte inleidende zin erboven
  - `FileInput.docs.md` toont beide vormen in de sectie "In form field context": lopende tekst in de description, en de lijst boven het form field wanneer het er te veel zijn voor een leesbare zin
