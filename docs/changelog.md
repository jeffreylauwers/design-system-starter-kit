...
- **Het icoon in een variant verliest zijn kleur, en dat is nog niet opgelost.** Na een tweede import staat het icoon op `color/neutral/color-default`, de eigen kleur van het icooncomponent, in plaats van op de kleur uit de spec. Twee verklaringen zijn geprobeerd en allebei in Figma weerlegd: het opnieuw zetten van de `INSTANCE_SWAP`-standaardwaarde, en de volgorde waarin de kleur en de properties geschreven worden.

  Wat er wél is veranderd: de kleur wordt nu als allerlaatste geschreven, ná het koppelen van de properties, en daarna teruggelezen. Een icoonlaag die zijn variable alsnog niet draagt gaat als waarschuwing de log in. Dat sluit de volgorde uit als oorzaak en levert de meting op om verder te zoeken. De standaardwaarde van een bestaande `INSTANCE_SWAP` blijft daarnaast staan, want wat Figma daar opslaat is niet per se de `key` die de plugin aanleverde, en die waarde is de keuze van de designer.

  De mock bootst de verwisseling nu na: haal de herhaling weg en de smoke test meldt `chevron-left.fills -> color/neutral/color-default (verwacht link/color)`, precies wat er in Figma te zien is. In Figma verandert de herhaling niets, dus het echte verschil zit in iets wat de mock nóg niet nadoet. Verder opgepakt in een eigen issue.
...
De hoogte animeert via `grid-template-rows` van `0fr` naar `1fr`, met `overflow: hidden` op het paneel. `padding-block` animeert mee van `0` naar zijn token-waarde: padding krimpt niet onder zijn eigen hoogte, dus zonder die transitie bleef er een strook van 16px staan waar het paneel nul hoog hoort te zijn.

`display` staat uitsluitend op de open-staat. Zou het op de basis-selector staan, dan wint die van de UA-regel `[hidden] { display: none }`, omdat auteurs-CSS de UA-stylesheet verslaat ongeacht specificiteit, en dan zou het paneel altijd zichtbaar zijn.

Omdat het paneel nu een grid-container is, kreeg `__search-inner` een expliciete `inline-size: 100%`. Een grid-item met `margin-inline: auto` krijgt namelijk fit-content-breedte in plaats van de volle breedte; zonder die regel werd het zoekveld enkele pixels smaller. Geometrie gemeten tegen `main` op 375px en 1400px: paneel, binnenlaag, invoerveld en knop staan op exact dezelfde posities en breedtes, en op 1400px blijft de binnenlaag netjes op 1200px gecentreerd.

Geen wijziging in markup of API. Het `hidden`-attribuut blijft de enige schakelaar, dus de HTML/CSS-laag krijgt dezelfde animatie zonder JavaScript. `prefers-reduced-motion: reduce` schakelt alles uit.
...
- **Tree-shaking werkte niet: één component importeren kostte evenveel als alle 73**: `components-react` declareerde geen `sideEffects`, en tsdown leverde de ESM-output als één gebundelde module. Bundlers konden daardoor niets weglaten. De ESM-build draait nu in `unbundle`-modus (één module per component) en alle vier de gebundelde packages declareren `"sideEffects": false`. Gemeten met Vite 6, geminificeerd:

  | Import               | Voor      | Na        |
  | -------------------- | --------- | --------- |
  | Alleen React         | 194,47 kB | 194,47 kB |
  | React + `Heading`    | 261,7 kB  | 195,02 kB |
  | React + `Button`     | 261,7 kB  | 214,05 kB |
  | React + alle exports | 263,24 kB | 263,24 kB |
  - `Heading` kost nu 0,55 kB in plaats van 68 kB. `Button` kost meer omdat het `Icon` importeert, en `Icon` de volledige iconenregistry meeneemt
  - Het aantal unieke `dsn-`klassen in een Button-only bundle daalde van 153 naar 8
  - **CommonJS blijft gebundeld.** In `unbundle`-modus laat tsdown per component een `require("./Button.cjs")` staan naar de weggestripte CSS-module, een bestand dat niet bestaat: 69 kapotte requires. De ESM-output heeft dat probleem niet. ESM en CJS worden daarom apart gebouwd; CJS-consumenten missen alleen het tree-shaking-voordeel
  - `sideEffects: false` klopt ook voor `components-web`: dat package registreert custom elements bewust niet op module-niveau, maar via `defineButton()` en verwanten

### Documentatie

#### Fixed

- **De belofte van tree-shaking klopte niet in 3.0.0**: zowel `Icon.docs.md` als de README van `components-react` stelden dat alleen wat je importeert in je bundle komt, terwijl één component importeren vrijwel evenveel kostte als alle 73. Beide teksten zijn herschreven; de oorzaak zelf is opgelost in de Packaging-sectie hierboven
...
- **Iconset naar Figma** (issue [#322](https://github.com/jeffreylauwers/design-system-starter-kit/issues/322)): de 51 Tabler-iconen uit `components-html/assets/icons/` worden nu als Figma-componenten gegenereerd. Tot nu toe bakte de componentgenerator de SVG in elke variant in: een Button met een chevron had 81 eigen kopieën van dat icoon, een designer kon het niet wisselen en een iconwijziging landde niet in bestaande instanties. Dit is bovendien de voorwaarde voor instance swap properties (issue [#323](https://github.com/jeffreylauwers/design-system-starter-kit/issues/323)).
  - Nieuwe generator `figma-sync/src/build-icons.js` en het script `pnpm build:figma-icons`, met `figma-sync/dist/icons.json` als uitvoer (`$schema: dsn-figma-icons/1`)
  - Bewust géén meting in een browser zoals bij de componenten: de bron is een SVG-bestand met een vaste `viewBox`, dus er valt niets te meten. Wel een normalisatie van de `<svg>`-tag, want 31 van de 51 bestanden dragen een `width` en 20 niet
  - 51 losse componenten op een eigen pagina `dsn/Icons`, geen component set met een `icon`-as: een instance swap property kiest uit componenten, en een set van 51 varianten levert in die keuzelijst één regel op waarna je alsnog via de variant-dropdown moet zoeken
  - De namen komen uit de bestandsnamen en worden tegen `icon-registry.generated.ts` aan gehouden; loopt de een uit de pas met de ander, dan komt dat als waarschuwing in de build. Een nieuw icoon in de assets-map komt er zonder handmatige stap bij
  - De vectoren krijgen `dsn/Primitives → color/neutral/color-default`, zodat de iconen de theme-schakelaar volgen. Dat is een gekozen standaard en geen gemeten binding: een losstaand icooncomponent heeft geen tekstouder om zijn kleur van te erven, dus de verificatie uit [DR-2026-06](./decisions/DR-2026-06-figma-bindingen-meten-plus-cssom.md) heeft hier niets om tegen af te zetten. Een instance mag de kleur overschrijven, zoals `currentColor` in de browser
  - **Idempotent op node-id.** Een tweede import werkt een bestaand icooncomponent bij door zijn vectoren te vervangen, niet door het component opnieuw aan te maken. Een nieuw component met dezelfde naam is voor Figma een ánder component, en dan raakt elke geplaatste instance los. De smoke test controleert de node-ids expliciet
  - Het frame dat `createNodeFromSvg` oplevert gaat eraf: in de iconset ís het component het icoon, dus een wrapperlaag ertussen voegt niets toe
  - Een icoon dat uit de assets-map verdwijnt blijft in Figma staan, met een melding. Automatisch verwijderen zou elke instance ervan detachen, en dat is een beslissing van een mens
  - De iconimport opent de pagina `dsn/Icons` niet: een component-import zet zijn set op `figma.currentPage`, dus dat zou een Button tussen de iconen laten belanden
  - De plugin verwerkt meerdere tegelijk gesleepte bestanden nu in de volgorde variables → iconen → componenten, en één voor één. Zonder die wachtrij hing het van de bestandsgrootte af of de afhankelijkheden op tijd bestonden

- **Component properties: icon-slots, instance swap en text** (issue [#323](https://github.com/jeffreylauwers/design-system-starter-kit/issues/323)): de generator leverde geometrie, styling en auto layout, maar geen component properties. Een designer moest die na élke import opnieuw met de hand aanmaken, en omdat elke import een nieuwe component set oplevert ging dat werk elke keer verloren. Dat maakte het in de praktijk het duurste onderdeel van de hele keten.
  - Een matrix declareert nu `componentProperties`, met een `slot` die naar een `data-figma-slot` in de gerenderde markup wijst. Die markering staat er omdat de plugin de laag anders op klassenaam zou moeten raden, en dan hangt een property stilletjes aan de verkeerde laag zodra de CSS-klasse verandert
  - Button krijgt vijf properties: `label` (text), `showIconStart` / `iconStart` en `showIconEnd` / `iconEnd` (boolean + instance swap). De namen volgen de React-props; omdat `iconStart` in code hét icoon ís, is dat hier de instance swap en heet de boolean `showIconStart`
  - Beide icoonslots worden in élke variant gerenderd en staan standaard uit. Een Figma-property kan alleen een laag aan- of uitzetten die er is, en een property op de set die in een variant geen laag heeft doet daar niets
  - **Iconen zijn nu instances van de icooncomponenten uit [#322](https://github.com/jeffreylauwers/design-system-starter-kit/issues/322)**, in élke component set en niet alleen in de slots. Een Button had 81 losse kopieën van dezelfde chevron; die volgen nu allemaal het icooncomponent. De gemeten kleur blijft er als override overheen liggen, dus een icoon in een `strong` Button is nog steeds wit
  - Alles wat niet gelegd kan worden gaat als fout de log in, met de reden: slot ontbreekt, laag is geen instance, icoon staat niet op `dsn/Icons`, of Figma weigerde de property. De smoke test heeft daar een eigen sectie voor, inclusief een import zonder iconset die moet melden in plaats van stil door te lopen
  - Nieuwe richtlijn in de README van `figma-sync`: een eigenschap die gemeten tokens verandert (kleur, padding, maat) wordt een variant-as, een eigenschap die alleen bepaalt of en welke laag er staat wordt een component property. Daarom is `iconOnly` bij Button géén property: die heeft eigen `padding-inline`-tokens en zou als boolean de padding van de tekstvariant houden
  - Onzekerheid die in Figma zelf bevestigd moet worden: of de standaardwaarde van een `INSTANCE_SWAP` de `key` of de node-id van het component is, verschilt per API-versie. De plugin probeert beide en meldt het als geen van beide wordt geaccepteerd

- **Elk icooncomponent krijgt dezelfde laagstructuur `Group > Shape`** (issue [#323](https://github.com/jeffreylauwers/design-system-starter-kit/issues/323)): een instance swap naar een ander icoon leverde een verkeerd gekleurd icoon op, zichtbaar als een donker glyph met een wit kadertje eromheen. Figma zoekt de overrides op een instance terug via het **laagpad**, en het aantal vectorlagen per icoon liep uiteen van 1 (`chevron-right`) tot 4 (`archive`, `alert-triangle`). Na een swap landde de kleuroverride daardoor op een andere laag dan bedoeld.
  - De 24x24-hulppath van Tabler (`<path stroke="none" d="M0 0h24v24H0z" fill="none"/>`, in 31 van de 51 bestanden) gaat er in de generator uit. Hij tekent niets en zet alleen de afmeting vast, die hier al uit de expliciete `width`/`height` komt
  - De plugin zet de lijnen met `outlineStroke()` om naar vlakken. Zonder die stap zit de kleur bij een lijn-icoon op `strokes` en bij een vlak-icoon op `fills`, en dan landt een override op `fills` na een swap tussen die twee nergens
...
- **Alert wees in Figma naar een niet-bestaand icoon** (issue [#323](https://github.com/jeffreylauwers/design-system-starter-kit/issues/323)): de matrix noemde het icoon van de `negative`-variant `alert-circle`, terwijl `Alert.tsx` `exclamation-circle` gebruikt en `alert-circle` helemaal niet in de assets-map bestaat. Code en Figma gebruikten dus verschillende termen voor dezelfde laag. Kwam aan het licht doordat de icoonlagen nu instances van echte icooncomponenten worden en de naam dus moet kloppen om er een te vinden.

#### Changed

- **Componentlagen volgen de theme-schakelaar** (issue [#321](https://github.com/jeffreylauwers/design-system-starter-kit/issues/321), [DR-2026-06](./decisions/DR-2026-06-figma-bindingen-meten-plus-cssom.md)): de variables en de component sets stonden los van elkaar. Een gegenereerde Button had een fill van `#1b59a4` in plaats van een verwijzing naar `dsn/Components → button/strong/background-color`, dus schakelen naar `start-dark` veranderde er niets aan. De lagen worden nu aan variables gebonden: 1661 bindingen over de vijf componenten.
  - Gebonden velden: `fills`, `strokes`, `strokeWeight`, de vier hoekradii, de vier paddings, `itemSpacing`, `minWidth`, `minHeight` en `fontSize`
...
- **`min-block-size` en `min-inline-size` komen mee naar Figma** (issue [#321](https://github.com/jeffreylauwers/design-system-starter-kit/issues/321)): een frame staat in Figma op HUG en rekent zijn maat opnieuw uit content plus padding, waarbij de gemeten hoogte wordt weggegooid. Een Button van de standaardgrootte werd daardoor 42px in plaats van de 48px die de CSS afdwingt, en zakte dus onder het aanraakdoel van [WCAG 2.5.5](https://www.w3.org/WAI/WCAG22/quickref/#target-size-minimum).
  - `minWidth` en `minHeight` staan nu in de node spec en worden aan hun token gebonden, dus `button/min-block-size` en de override `button/size/small/min-block-size` (40px) werken allebei door
  - Ze bestaan in Figma alleen op een auto-layout frame; elders komen ze in het bindingsreport in plaats van stil te verdwijnen
