...
- **Het icoon in een variant verloor zijn kleur.** Niet de standaardwaarde bleek de trigger maar het koppelen zelf: `componentPropertyReferences.mainComponent` zetten is voor Figma een verwisseling, en een verwisseling gooit de overrides op de geneste lagen weg. Het icoon viel daarmee terug op `color/neutral/color-default`, de eigen kleur van het icooncomponent. De kleuren worden nu na afloop van álles nog een keer gezet en daarna teruggelezen; een laag die zijn variable alsnog niet draagt gaat als waarschuwing de log in. Daarnaast blijft de standaardwaarde van een bestaande `INSTANCE_SWAP` staan: wat Figma daar opslaat is niet per se de `key` die de plugin aanleverde, dus erop vergelijken helpt niet, en hem opnieuw zetten is weer een verwisseling.

  De mock bootst die verwisseling nu na, en daarmee is dit voor het eerst te testen: zonder de herhaling meldt de smoke test `chevron-left.fills -> color/neutral/color-default (verwacht link/color)`, exact wat er in Figma te zien was. Dat is de derde keer deze PR dat een bug alleen bestond in het gat tussen de mock en de Plugin API.
